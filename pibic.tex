...
semi-anel em que o produto tenha escopo disjunto~\cite{sp-theorem}. Em aplicações, SPNs tiveram
resultados impressionantes em diversas aplicações, como enovelamento de
proteínas~\cite{rec-dec-non-convex}, modelagem de sinais~\cite{model-speech}, classificação e
reconstrução de imagens \cite{gens-domingos,poon-domingos,clustering}, reconhecimento de
atividade~\cite{activity} e linguagem natural~\cite{nat-lang}.
